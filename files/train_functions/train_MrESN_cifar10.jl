...
function __fill_X_MrESN_cifar10!(mrE, args::Dict )
    ...
    for t in 1:args[:train_length]
        for i in 1:length(mrE.esns)
            for _ in 1:args[:initial_transient]
                update_cifar(mrE.esns[i], args[:train_data][:,:,:,t], f )
            end
            update_cifar(mrE.esns[i], args[:train_data][:,:,:,t], f )
        end
...
function __make_Rout_MrESN_cifar10!(mrE,args)
    X             = mrE.X
...
function __do_train_MrESN_cifar10!(mrE, args)
    num   = args[:train_length]
    mrE.X = zeros( sum([esn.R_size for esn in mrE.esns]) + args[:image_size][1]*args[:image_size][2]*3 + 1, num)
    ...
    __fill_X_MrESN_cifar10!(mrE,args)
    __make_Rout_MrESN_cifar10!(mrE,args)
end
