...
_params = Dict{Symbol,Any}(
     :gpu           => true
    ,:wb            => false
    ,:wb_logger_name=> "MRESN_pso_R+_Mnist_GPU"
...
function do_batch( _params,_a,_d,_r,_s,_i )
    ...
    esns = [
            ESN( 
                 R      = new_R(_params_esn[:nodes][i], density=_d, rho=_r, gpu=true, bounds=(0,1))
...
for _it in 1:repit
    ...
    lx = [0.001, 0.001, 0.001, 0.01, 1.0 ]'
    ux = [1.5,   0.7,   1.5,   1.5,  4.0 ]'
    lx_ux = vcat(lx,ux)
